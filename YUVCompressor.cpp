...
static void calculateDCTMatrix(float DCT_matrix[size * size]) {
  const float inv_size_sqrt = 1.0f / std::sqrt(size);
  const float inv_size_float = 1.0f / static_cast<float>(size);
  const float sqrt_2 = std::sqrt(2.0f);
  for (size_t j = 0; j < size; j++) {
    DCT_matrix[j * size] = inv_size_sqrt;
    for (size_t i = 1; i < size; i++) {
      DCT_matrix[i + j * size] = sqrt_2 * inv_size_sqrt * std::cos(static_cast<float>((2*j+1)*i) * M_PIf32 * 0.5f * inv_size_float);
    }
  }
}
...
static void applyDCTtoBlock(float data_block[DCT_matrix_size * DCT_matrix_size], const float DCT_matrix[DCT_matrix_size * DCT_matrix_size], int8_t res[DCT_matrix_size * DCT_matrix_size], const float q_table[DCT_matrix_size * DCT_matrix_size]) {
  ...
  for (int i = 0; i < DCT_matrix_size * DCT_matrix_size; i++) {
    res[i] = std::round(data_block[i] / q_table[i]);
  }
}
...
static void applyDCTtoPane(uint8_t** res, uint8_t* res_size, const uint8_t* data, const int& width, const int& height, const float DCT_matrix[DCT_matrix_size * DCT_matrix_size], const float q, const float q_50_table[DCT_matrix_size * DCT_matrix_size]) {
  ...
  const float q_table_mul = (q >= 50.0f) ? (100.0f - q) / 50.0f : 50.0f / q;
  for (int i = 0; i < DCT_matrix_size * DCT_matrix_size; i++) {
    ...
    for (int i = 0; i < width; i += DCT_matrix_size) {
      ...
      for (int jj = 0; jj < DCT_matrix_size; jj++) {
        for (int ii = 0; ii < DCT_matrix_size; ii++) {
          data_block[ii + jj * DCT_matrix_size] = data[(i + ii) + (j + jj) * width] - 128;
        }
      }
...
template<int DCT_matrix_size>
static void restoreDCTBlock(const uint8_t* huffman, const uint8_t& huffman_size, const float DCT_matrix[DCT_matrix_size * DCT_matrix_size], float res[DCT_matrix_size * DCT_matrix_size], const uint8_t q_table[DCT_matrix_size * DCT_matrix_size]) {
...
template<int DCT_matrix_size>
static void restoreDCTPane(uint8_t* data, const uint8_t* huffman_data, const int& huffman_data_size, const int& width, const int& height, const float DCT_matrix[DCT_matrix_size * DCT_matrix_size], const float q, const float q_50_table[DCT_matrix_size * DCT_matrix_size]) {
  assert(DCT_matrix_size > 0);
  assert(width % DCT_matrix_size == 0);
  assert(height % DCT_matrix_size == 0);
  // Calculate quality matrix
  uint8_t q_table[DCT_matrix_size * DCT_matrix_size];
  const float q_table_mul = (q >= 50.0f) ? (100.0f - q) / 50.0f : 50.0f / q;
  for (int i = 0; i < DCT_matrix_size * DCT_matrix_size; i++) {
    q_table[i] = std::clamp<uint8_t>(static_cast<uint8_t>(std::round(q_50_table[i] * q_table_mul)), 1, 255);
  }
  // Split into blocks
  int huffman_index = 0;
  for (int j = 0; j < height; j += DCT_matrix_size) {
    for (int i = 0; i < width; i += DCT_matrix_size) {
      ...
      huffman_index += huffman_data[huffman_index] + 1;
      assert(huffman_index <= huffman_data_size);
      for (int jj = 0; jj < DCT_matrix_size; jj++) {
        for (int ii = 0; ii < DCT_matrix_size; ii++) {
          data[(i + ii) + (j + jj) * width] = static_cast<uint8_t>(std::round(block_res[ii + jj * DCT_matrix_size])) + 128;
        }
      }
    }
  }
...
extern void test_dct() {
  ...
  restoreDCTBlock<8>(huffman_dump, huffman_size, DCT_matrix_8, block_res2, q_table2);
  for (int i = 0; i < 64; i++) {
    res[i] = static_cast<uint8_t>(std::round(block_res2[i])) + 128;
  }
  for (int j = 0; j < 8; j++) {
    for (int i = 0; i < 8; i++) {
...
void YUVCompressor::DST_IYUV_compress(MyYUV& myyuv, const uint8_t q[3]) {
  ...
  for (int i = 0; i < 3; i++) {
    if (q[i] < 1 || q[i] > 99) {
      throw std::runtime_error("Level of quality must be between 1 and 99");
    }
  }
  ...
  //std::cout << tmp_count << '\n';
  std::cout << "DCT IYVV compressed size " << total_size << '\n';
  delete[] myyuv.data;
...
void YUVCompressor::DST_IYUV_decompress(MyYUV& myyuv) {
  ...
  //std::cout << k << '\n';
  restoreDCTPane<8>(Y, myyuv.data, k, myyuv.width, myyuv.height, DCT_matrix_8, q[0], lum_q_table);
  saved_k = k;
  for (int i = 0; i < myyuv.width * myyuv.height / 256; i++) {
    k += myyuv.data[k] + 1;
  }
  //std::cout << k - saved_k << '\n';
  restoreDCTPane<8>(U, myyuv.data + saved_k, k - saved_k, myyuv.width / 2, myyuv.height / 2, DCT_matrix_8, q[1], chroma_q_table);
  saved_k = k;
  for (int i = 0; i < myyuv.width * myyuv.height / 256; i++) {
    k += myyuv.data[k] + 1;
  }
  //std::cout << k - saved_k << '\n';
  restoreDCTPane<8>(V, myyuv.data + saved_k, k - saved_k, myyuv.width / 2, myyuv.height / 2, DCT_matrix_8, q[2], chroma_q_table);
  delete[] myyuv.data;
  myyuv.data = data;
...
void YUVCompressor::compress(const std::string& type, const std::vector<std::string>& params) {
  ...
  if (search != compression_map.end()) {
    if (isCompressed()) {
      std::cout << "Warning. Already compressed. Multiple compressions not supported.\n";
      decompress();
    }
    return search->second(*this, params);
  } else {
    throw std::runtime_error(type + " not found");
  }
}
